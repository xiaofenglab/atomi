...
def render_uo2_thermal_stress_input(
    *,
    ...
    return f"""# Generated by atomi moose-uo2-thermal-stress
# Use moose-qha-md-material first to create {material_include}.
# Geometry is a 2D axisymmetric R-Z standalone UO2 pellet.
...
[Materials]
  ...
  [thermal_expansion]
    ...
    thermal_expansion_function = uo2_alpha
...
def uo2_thermal_stress_main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Write a starter UO2 pellet thermal-stress MOOSE input that consumes "
            "moose-qha-md-material CSV/[Functions] output."
        ),
    )
    parser.add_argument("--material-csv", type=Path, default=Path("uo2_moose_material_properties.csv"))
    parser.add_argument("--material-include", type=Path, default=Path("uo2_material_functions.i"))
    parser.add_argument("--output", type=Path, default=Path("uo2_pellet_thermal_stress.i"))
    ...
    input_text = render_uo2_thermal_stress_input(
        ...
        stress_free_temperature_k=args.stress_free_T,
