"""Shared thermodynamic exchange schemas.

These dataclasses are intentionally lightweight and dependency-free. They sit
above the zentropy/CALPHAD/MOOSE-specific layers so atomistic evidence can move
between modules without each module inventing a private table contract.
"""
...
def write_ce_training_jsonl(path: Path, training_set: CETrainingSet) -> None:
    """Write a backend-neutral CE/MC training set as JSONL."""
...
def read_ce_training_jsonl(path: Path) -> CETrainingSet:
    """Read a backend-neutral CE/MC training set from JSONL."""
