...
""",
    ...
    _write_text(
        ...
        f"""
# Legacy `wf` projector route. In stock QE documentation this reads a
# `{args.prefix}.hub` file generated by `pmw.x`; it is not an MLWF route.
# Tesch and Kowalski (2022) used this application layer with U calculated
# separately by linear response, not with the pmw.x projectors themselves.
HUBBARD (wf)
U {args.element}-{args.manifold} U_FROM_SEPARATE_PIOTR_2022_STYLE_LR
""",
    )
    _write_text(
        outdir / "MLWF_ROUTE.md",
        f"""
# Maximally localized Wannier Hubbard projectors for {args.element} {args.manifold}

Use QE 7.5 or a newer validated build and follow its shipped
`PP/examples/example07` exactly:

1. Run `pw.x` SCF/NSCF with a validated magnetic UO2 branch.
2. Run `pw2wannier90.x` and `wannier90.x`; inspect interpolation, spreads,
   centers, and U-5f/O-2p character.
3. Run `wannier2pw.x` to convert the accepted MLWFs into QE Hubbard
   projectors.
4. Calculate U with an explicitly validated matched response implementation
   using those same projectors. Stock `hp.x` is not assumed to provide this
   matched MLWF U. Do not attribute this step to Piotr without pinning the
   collaborator implementation.

Do not substitute the legacy `pmw.x`/`HUBBARD (wf)` path for this MLWF route.
""",
    )
    _write_text(
        outdir / "WORKFLOW.md",
...
QE has two distinct routes that must not be conflated. The legacy
`HUBBARD (wf)` route reads a `pmw.x` projector file and is not maximally
localized. Tesch and Kowalski (2022) calculated U separately by linear response,
then used `pmw.x` projectors for single-point DFT+U; their Wannier projectors
were not used in the U calculation. The modern MLWF route uses `pw2wannier90.x`, Wannier90, and
`wannier2pw.x`; use the exact example shipped with the selected QE release.
Stock `hp.x` does not establish a matched U for the same MLWF projector. A matched
research response branch is required; if it is Piotr's newer implementation,
record its commit and equations before using that label.
...
""",
    ...
    metadata = {
        "schema": "atomi.qe_hubbard_u.v2",
        "prefix": args.prefix,
        "baseline": "QE hp.x with ortho-atomic projectors",
        "legacy_wf": "Piotr 2022 application layer: pmw.x projectors with separately calculated LR U",
        "experimental": "QE 7.5+ MLWF projector via Wannier90/wannier2pw.x plus matched response",
