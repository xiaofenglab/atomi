...
HUBBARD (wf)
U {args.element}-{args.manifold} U_FROM_MATCHED_PMW_RESPONSE
""",
    )
    _write_text(
...
3. Run `wannier2pw.x` to convert the accepted MLWFs into QE Hubbard
   projectors.
4. Calculate U with Piotr's matched response implementation using those same
   projectors. Stock `hp.x` is not assumed to provide this matched MLWF U.
...
localized. The modern MLWF route uses `pw2wannier90.x`, Wannier90, and
...
Piotr/research response branch or explicitly validated alternative is required;
record its commit and equations.
...
""",
    ...
    metadata = {
        ...
        "legacy_wf": "pmw.x projector file; not maximally localized",
