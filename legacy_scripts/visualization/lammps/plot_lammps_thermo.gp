...
#
# Usage:
#   gnuplot -e "file='log.lammps'" plot_lammps_thermo.gp
# ------------------------------------------------------------
...
base = "awk 'BEGIN{f=0} /^ *Step/{f=1; next} f && NF>0 && $1+0==$1 {print}' ".file

# Count rows
ncmd = "awk 'BEGIN{f=0;n=0} /^ *Step/{f=1; next} f && NF>0 && $1+0==$1 {n++} END{print n}' ".file
