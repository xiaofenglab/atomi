...
Experimental PDF matching starts with `pdf_md_compare` for ranking MD-derived G(r), S(Q), F(Q), and iQ=S(Q)-1 curves against PDFgetX/PDFgui/RMC-style two-column data, followed by `pdf_md_reweight` for conservative maximum-entropy-style reweighting of MD temperature/window candidates. `pdf_md_compare` can also prepare or run raw `.chi` sample/empty/background/reference reductions with `pdfgetx3`, deriving composition and density from the selected MD box when available, or using user/API density overrides. Empty-container and background files are recorded as the implicit carriers of container/instrument/background corrections. The raw workflow writes `pdfgetx3_exp/logs`, native and CSV reduced data in `pdfgetx3_exp/final_data`, publication-style PNG/PDF quick plots in `pdfgetx3_exp/plots`, and process metadata/logs; PDFgetX `.iq` comparisons are treated as reduced/pseudo intensity comparisons, not reconstructed raw detector intensity.
