...
MD-ensemble XAFS starts with `xafs_lammps_prepare`, which reuses the PDF/LAMMPS frame-window machinery to write absorber-centered FEFF inputs from one MD run, a config JSON, an MD root, or an existing `pdf_lammps` output. The default absorber is the first metal in the selected frame, and `xraydb` is used for edge metadata when available. `xafs_larch_run` then runs or collects FEFF/Larch `chi(k)` outputs and averages the MD frame/site ensemble; `xafs_md_compare` overlays the resulting `chi(k)` against experimental XAFS data. Larch is included as a Python dependency for Python >=3.10 through `xraylarch`, while FEFF execution still depends on a usable `feff8l`/`feff6l` executable in the active environment. The roadmap is to combine `pdf_md_compare`/`pdf_md_reweight` with absorber-specific XAFS ranking so one MD ensemble can be constrained by both total scattering PDF and edge-specific XAFS.
